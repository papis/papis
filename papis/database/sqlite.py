...
def _make_sqlite_schema(table: str, columns: Sequence[str]) -> str:
    ...
    # - Still a fixed number of searchable columns (see :confval:`sqlite-schema-field`).
...
class SQLiteDatabase(Database):
    ...
    def _create_tables(self) -> None:
        ...
        if any(name in SQLITE_RESERVED_COLUMNS for name in field_names):
            raise ValueError(
                f"one of the field names is reserved: {field_names} "
