...
@library_router.get(
    "/import",
    tags=["Import"],
    response_model=ImportersResponse,
...
@library_router.post(
    "/import/match",
    response_model=ImporterMatchResponse,
    tags=["Import"],
    responses=exceptions.ResourceNotFoundError.responses(
        types=[exceptions.ErrorCode.LIBRARY_NOT_FOUND]
    ),
...
async def fetch_from_importers(
    ...
    if importers is not None:
        ...
        if missing:
            raise exceptions.ResourceNotFoundError(
                ...
                type=exceptions.ErrorCode.IMPORTER_NOT_FOUND,
    ...
    if not matched:
        ...
        raise exceptions.BadRequestError(
            ...
            type=exceptions.ErrorCode.INVALID_URI,
    ...
    if not fetched:
        raise exceptions.UpstreamError(
            ...
            type=exceptions.ErrorCode.UPSTREAM_ERROR,
    ...
    if not merged.data:
        raise exceptions.BadRequestError(
            ...
            type=exceptions.ErrorCode.IMPORTER_NO_DATA,
            context={"uri": uri, "importers": matched_names},
        )

    return ImporterFetchResponse(metadata=merged.data)
