...
from fastapi import Body, File, Form, Header, Path, Query, Request, Response, UploadFile
...
@library_router.post(
    ...
    responses={
        ...
        **exceptions.BadRequestError.responses(
            types=[
                exceptions.ErrorCode.MISSING_FIELD,
...
async def add_file(
    ...
    A suffix is added as needed to resolve file name collisions.
    """
    ...
    if file_upload is None and link_file is None:
        raise exceptions.BadRequestError(
            "Either 'file_upload' or 'link_file' is required.",
            type=exceptions.ErrorCode.MISSING_FIELD,
            context={"fields": ["file_upload", "link_file"]},
        )
...
async def download_file(
    ...
    include an ``ETag`` based on the file's modification time. If the client sends a
    request with a ``If-None-Match`` matching the file's modification time, a
    ``304 Not Modified`` response is returned instead of the file.
    """
    db = get_db(library)
    doc = get_doc(db, id)
    doc_folder = get_folder(doc)
