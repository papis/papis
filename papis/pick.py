...
def get_picker(name: str) -> Optional[Type[Picker[Any]]]:
    ...
    picker = get_plugin_by_name(PICKER_EXTENSION_NAME, name)
    if picker is None:
        logger.error("Failed to load picker '%s'. "
                     "Falling back to default 'papis' picker!", name)
        picker = get_plugin_by_name(PICKER_EXTENSION_NAME, "papis")
...
def pick(items: Sequence[T],
    ...
    picker: Optional[Type[Picker[T]]] = get_picker(name)
    if picker is None:
        return []
