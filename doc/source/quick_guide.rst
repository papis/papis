...
configurable) way, but if you want to move or rename them manually, you're free
to do so. As with the Unix ``mv`` command, moving and renaming are the same
thing: renaming a folder is just moving it within the same directory. Papis
therefore offers a single ``mv`` command for both, which can be used to change
the name or location of a document's folder.
...
Alternatively, you could also use the ``--to`` option to move the folder to
a name or location of your choosing. Finally, you could also adjust the
