...
def test_download_note_not_found(
...
def test_add_note(client: TestClient, tmp_config: TemporaryConfiguration) -> None:
...
def test_add_note_with_template(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
    """POST .../notes generates content from notes-template."""
    import papis.config

    papis_id, path = create_test_document(client, title="Template Test Document")
    ...
    with open(note_path, encoding="utf-8") as f:
        content = f.read()
    assert "Title: Template Test Document" in content


def test_add_note_when_exists(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
...
def test_download_note(client: TestClient, tmp_config: TemporaryConfiguration) -> None:
...
def test_replace_note(client: TestClient, tmp_config: TemporaryConfiguration) -> None:
...
def test_replace_note_not_found(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
...
def test_delete_note(client: TestClient, tmp_config: TemporaryConfiguration) -> None:
...
def test_delete_note_missing_on_disk(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
...
def test_rename_note(client: TestClient, tmp_config: TemporaryConfiguration) -> None:
...
def test_rename_note_self_rename(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
...
def test_rename_note_not_found(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
...
def test_rename_note_path_normalization(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
...
def test_rename_note_conflict(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
    """PATCH .../notes returns 409 when target filename already exists."""
...
def test_download_note_etag(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
    """GET .../notes returns an ETag header based on file mtime."""
...
def test_download_note_if_none_match_304(
    client: TestClient,
    tmp_config: TemporaryConfiguration,
) -> None:
    ...
    r1 = client.get(f"/api/v1/libraries/test/documents/{papis_id}/notes")
...
def test_download_note_if_none_match_mismatch(
    client: TestClient,
    tmp_config: TemporaryConfiguration,
) -> None:
