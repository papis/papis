...
def test_list_importers(client: TestClient, tmp_config: TemporaryConfiguration) -> None:
...
def test_match_importers_multiple(
...
def test_fetch_unknown_importer_returns_404(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
    ...
    with patch(
        ...
        assert response.json()["type"] == "/errors/importer-not-found"
...
def test_fetch_filesystem_importer_rejected(
    ...
    with patch(
        ...
        assert response.json()["type"] == "/errors/local-mode-required"
    ...
    with patch(
        ...
        assert response.status_code == 400
        assert "folder" in response.json()["detail"]
        assert "lib" in response.json()["detail"]
...
def test_fetch_no_importers_match_returns_400(
    ...
    with (
        ...
        assert response.status_code == 400
        assert response.json()["type"] == "/errors/invalid-uri"
...
def test_fetch_network_error_returns_502(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
...
def test_fetch_importer_returns_no_data_returns_400(
    ...
    with (
        ...
        assert response.status_code == 400
        assert response.json()["type"] == "/errors/importer-no-data"
...
def test_fetch_auto_match_returns_data(
    ...
    with (
        ...
        assert response.status_code == 200
        body = response.json()
        assert body["metadata"]["title"] == "Auto Matched"
...
def test_fetch_multiple_importers_merges_results(
    ...
    with (
        ...
        assert body["metadata"]["title"] == "From Crossref"
        assert body["metadata"]["doi"] == "10.1234/test"
        assert body["metadata"]["author"] == "Smith"
...
def test_fetch_strips_ref_key(
    ...
    with (
        ...
        assert "ref" not in body["metadata"]
