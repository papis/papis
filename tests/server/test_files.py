...
def test_list_files(client: TestClient, tmp_config: TemporaryConfiguration) -> None:
...
def test_add_file(client: TestClient, tmp_config: TemporaryConfiguration) -> None:
...
def test_add_file_missing_source(
    ...
    """POST .../files returns 400 when neither file_upload nor link_file is provided."""
    ...
    data = response.json()
    assert data["type"] == "/errors/missing-field"
    assert "file_upload" in data["context"]["fields"]
    assert "link_file" in data["context"]["fields"]


def test_add_file_path_normalization(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
...
def test_add_file_unique_name_suffix(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
...
def test_add_file_respects_config(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
...
def test_download_file(client: TestClient, tmp_config: TemporaryConfiguration) -> None:
...
def test_download_file_with_special_chars(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
...
def test_replace_file(client: TestClient, tmp_config: TemporaryConfiguration) -> None:
...
def test_replace_file_with_special_chars(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
...
def test_delete_file(client: TestClient, tmp_config: TemporaryConfiguration) -> None:
...
def test_delete_file_missing_on_disk(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
...
def test_delete_file_with_special_chars(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
...
def test_rename_file(client: TestClient, tmp_config: TemporaryConfiguration) -> None:
...
def test_rename_file_preserves_extension(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
...
def test_rename_file_path_normalization(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
...
def test_rename_file_self_rename(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
...
def test_rename_file_conflict(
    ...
    """PATCH .../files returns 409 when target filename already exists."""
    ...
    assert response.json()["type"] == "/errors/file-exists"


# =============================================================================
# ETag headers on mutation endpoints
# =============================================================================


def test_download_file_etag(
    client: TestClient, tmp_config: TemporaryConfiguration
) -> None:
    """GET .../files/{file} returns an ETag header based on file mtime."""

    papis_id, _ = create_test_document(client)

    client.post(
        f"/api/v1/libraries/test/documents/{papis_id}/files",
        files={"file_upload": ("dl-etag.pdf", BytesIO(b"Content"), "application/pdf")},
    )

    response = client.get(
        f"/api/v1/libraries/test/documents/{papis_id}/files/dl-etag.pdf"
    )
    assert response.status_code == 200
...
def test_download_file_if_none_match_304(
    client: TestClient,
    tmp_config: TemporaryConfiguration,
) -> None:
    """If-None-Match matching ETag returns 304."""

    papis_id, _ = create_test_document(client)

    client.post(
        f"/api/v1/libraries/test/documents/{papis_id}/files",
        files={"file_upload": ("cache.pdf", BytesIO(b"Content"), "application/pdf")},
    )

    # First download to get ETag
    r1 = client.get(f"/api/v1/libraries/test/documents/{papis_id}/files/cache.pdf")
    assert r1.status_code == 200
    etag = r1.headers["etag"]

    # Second download with If-None-Match
    r2 = client.get(
        f"/api/v1/libraries/test/documents/{papis_id}/files/cache.pdf",
        headers={"If-None-Match": etag},
    )
    assert r2.status_code == 304


def test_download_file_if_none_match_mismatch(
    client: TestClient,
    tmp_config: TemporaryConfiguration,
) -> None:
    """If-None-Match non-matching ETag returns 200 with body."""

    papis_id, _ = create_test_document(client)

    client.post(
        f"/api/v1/libraries/test/documents/{papis_id}/files",
        files={"file_upload": ("mismatch.pdf", BytesIO(b"Content"), "application/pdf")},
    )

    response = client.get(
        f"/api/v1/libraries/test/documents/{papis_id}/files/mismatch.pdf",
